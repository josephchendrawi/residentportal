        ...
        public ActionResult Add(Document Model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    var result = DocumentBLL.CreateDocument(AccountNoteVO);
        ...
        public ActionResult Edit(Document Model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    var AccountNoteVO = DocumentBLL.Get(Model.AccountNoteId);
                    AccountNoteVO.NOTE = Model.Content;
                    AccountNoteVO.NAME = Model.Name;
                    AccountNoteVO.LAST_UPD_BY = UserId;

                    DocumentBLL.UpdateDocument(AccountNoteVO);

                    TempData["Message"] = "Successfully done.";
                    return RedirectToAction("View", "Documents", new { Id = Model.AccountNoteId });
                }
                catch (Exception ex)
                {
                    TempData["Message"] = ex.Message;
