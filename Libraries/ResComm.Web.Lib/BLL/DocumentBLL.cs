...
namespace ResComm.Web.Lib.BLL
{
    public class DocumentBLL
    {
        ...
        public List<AccountNoteVO> GetDocumentsByCustomerId(long CustomerId, int startIdx, int length, ref int TotalCount, string orderBy = "", string orderDirection = "", AccountNoteVO qFilter = null)
        {
            int Total_Count = 0;

            qFilter.ACCNT_ID = CustomerId;
            qFilter.ITEM_TYPE = ((int)ACCNT_NOTE_TYPE.HouseRule).ToString();
        ...
        public long CreateDocument(AccountNoteVO AccountNoteVO)
        {
            AccountNoteVO.ITEM_TYPE = ((int)ACCNT_NOTE_TYPE.HouseRule).ToString();
            return AccountNoteDALImpl.CreateAccountNote(AccountNoteVO);
        }

        public void UpdateDocument(AccountNoteVO AccountNoteVO)
        {
